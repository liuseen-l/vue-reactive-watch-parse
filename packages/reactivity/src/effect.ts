...
function triggerEffect(
  ...
  if (effect !== activeEffect) {
    ...
    // 只有执行run的时候一定会设置activeEffect,而执行调度器scheduler不一定会设置activeEffect，因为调度器内部可能会执行某些effect的run方法，也可能不执行相关run方法
    // 如果内部没有执行run方法，那么不会设置activeEffect,因此不会执行依赖收集的过程，内部访问响应式数据不会收集依赖，因为当前的activeEffect = null
    if (effect.scheduler) {
      effect.scheduler()
    } else {
      effect.run()
